...
{
	case "file":
		file_put_contents($settings['log_file'], date("Y-m-d H:i:s").": ".$msg."\n",FILE_APPEND);
        break;
	case "db":
			...
			mysqli_query($con,"INSERT INTO ".$settings['log_db_table']." (`date`, `sign`, `log`) VALUES ('".date("Y-m-d H:i:s")."','".$sign."','".$msg."')");
			mysqli_close($con);
		}
	break;
	default:
...
// ============================================
$upfile2 = file_get_contents($tempeset.'update.ver');
$pie = explode('[PCUVER]', $upfile2);
$strfile = explode("\r\n", $pie[1]);
$nl = count($strfile);
for ($l=3;$l<$nl;$l++){
